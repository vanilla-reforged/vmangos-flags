...
export function globalSearch(query: string, limit = 80): SearchResult[] {
  ...
  for (const row of spellFamilyFlags) if (text(row.ability, row.comment, row.className, row.familyId, row.bitIndex).includes(q)) add({ title: row.ability, subtitle: `${row.className} · SpellFamilyFlag ${row.familyId} · bit ${row.bitIndex}`, path: `/spell-family-flags?family=${row.familyId}`, kind: 'Spell Family Flag' });
