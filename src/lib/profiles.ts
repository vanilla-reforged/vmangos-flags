...
export type Profile = {
  ...
  author?: string;
  description?: string;
  overrides: { spellFamilyFlags: Record<string, FamilyOverride> };
...
export function emptyProfile(name = 'Default'): Profile {
  ...
  return { id: uuid(), name, overrides: { spellFamilyFlags: {} }, presets: {}, createdAt: now, modifiedAt: now };
}
...
export function fromExport(data: ProfileExport, copy = false): Profile {
  const now = new Date().toISOString();
  return { ...data.profile, id: copy ? uuid() : data.profile.id, name: copy ? `${data.profile.name} (copy)` : data.profile.name, overrides: JSON.parse(JSON.stringify(data.overrides)), presets: JSON.parse(JSON.stringify(data.presets)), modifiedAt: now };
}
